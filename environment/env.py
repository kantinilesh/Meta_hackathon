...
# Each task activates exactly one wildcard so the reward logic fires correctly.
TASK_FLAGS = {
...
class ContractEnv:
    ...
    def state(self) -> Dict[str, Any]:
        return {
            "turn": self.turn,
            "max_turns": self.max_turns,
            "clauses": {c.id: c.model_dump() for c in self.clauses.values()},
    ...
    def step(self, action: Action) -> Tuple[Observation, Reward, bool, Dict[str, Any]]:
        ...
        if not clause:
            return (
                ...
                Reward(value=-0.1, breakdown=reward_breakdown, done=done, info={"error": "Invalid clause id"}),
                done,
                info,
            )
        ...
        # 2. Strict Deal-Breaker / Action enforcement
        if action.action_type == "terminate_deal":
            ...
            # task1 — Hidden lawsuit check (clause c2)
            ...
            if self.evidence_bomb_enabled and action.clause_id == "c4":
                ...
                if "gross negligence" in pt_lower:
                    if speaker == "seller_agent":
                        ...
                        reward_breakdown.proposal_quality += 0.5        # client succeeded
            ...
            if cp_resp.response_type == "accept":
                ...
                reward_breakdown.proposal_quality += 0.5
            elif cp_resp.response_type == "reject":
                reward_breakdown.proposal_quality -= 0.1
            elif cp_resp.response_type == "counter":
                clause.current_proposed_text = cp_resp.modified_text
            ...
            self.history.append(NegotiationTurn(
                turn_number=self.turn,
                speaker="client_agent" if getattr(action, "speaker", "seller_agent") == "seller_agent" else "seller_agent",
